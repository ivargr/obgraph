...
cdef traverse_graph_by_following_nodes(graph, follow_nodes, only_add_nodes_in_follow_nodes=False):
    ...
    while True:
        ...
        if len(next_nodes) == 1:
            ...
            if next_node is None:
                ...
                assert len(next_node) == 1, "There are multiple empty nodes from node %d: %s" % (
                current_node, next_nodes)
                next_node = next_node[0]

        assert next_node is not None, "Could not find any new nodes from %d. Edges are %s" % (
        current_node, str(next_nodes))
